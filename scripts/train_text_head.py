...
from phisdom.data.new_heads import TextSeqDataset, PaddedSeqCollator
...
def main():
    ap = argparse.ArgumentParser(description="Train Text Char-CNN head and calibrate (title+visible)")
    ap.add_argument("--train-jsonl", required=True)
    ap.add_argument("--val-jsonl", required=True)
    ap.add_argument("--output-dir", default="artifacts/text_head")
    ...
    tr_ds = TextSeqDataset(args.train_jsonl, max_len=args.max_len)
    va_ds = TextSeqDataset(args.val_jsonl, max_len=args.max_len)
    ...
    model = TextCharCNN(dropout=args.dropout).to(device)
